...
public abstract class FrequencyScreen extends Screen
{
	...
	{
		super(new TranslatableText(TranslationKeys.GUI_FREQUENCY_TITLE));
		this.frequency = frequency;
	}

	private ButtonWidget addFrequencyButton(int x, int y, int value)
	{
		return addDrawableChild(new ButtonWidget(x, y, WIDGET_WIDTH, WIDGET_HEIGHT, new LiteralText(""), button ->
		{
			int frequency = 0;
	...
	private void updateFrequencyButtonDesc()
	{
		if (hasShiftDown())
		{
			add1.setMessage(new LiteralText("+100"));
			add10.setMessage(new LiteralText("+1000"));
			sub1.setMessage(new LiteralText("-100"));
			sub10.setMessage(new LiteralText("-1000"));
		}
		else
		{
			add1.setMessage(new LiteralText("+1"));
			add10.setMessage(new LiteralText("+10"));
			sub1.setMessage(new LiteralText("-1"));
			sub10.setMessage(new LiteralText("-10"));
		}
	}
	...
	protected void init()
	{
		...
		done = addDrawableChild(new ButtonWidget((width - WIDGET_WIDTH) / 2, frequencyInput.y + WIDGET_HEIGHT + 20,
			WIDGET_WIDTH, WIDGET_HEIGHT,new TranslatableText("gui.done"), button ->
		{
			setFrequency();
			close();
		}));
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float delta)
	{
		...
		textRenderer.draw(matrices, title, (width - textRenderer.getWidth(title)) / 2, frequencyInput.y - 30, 0xFFFFFF);
	}
