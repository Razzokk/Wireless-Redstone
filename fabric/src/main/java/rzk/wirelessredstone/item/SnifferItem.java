...
public class SnifferItem extends FrequencyItem
{
	...
	public TypedActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand)
	{
		...
		{
			if (world.isClient)
				player.sendMessage(new TranslatableText(TranslationKeys.MESSAGE_NO_FREQUENCY).formatted(Formatting.RED), false);
		...
		if (!world.isClient)
		{
			...
			MutableText frequencyComponent = new LiteralText(String.valueOf(frequency)).formatted(Formatting.AQUA);

			if (transmitters.isEmpty())
			{
				player.sendMessage(new TranslatableText(TranslationKeys.MESSAGE_TRANSMITTERS_EMPTY, frequencyComponent), false);
				removeHighlightBlocks(stack);
			}
			else
			{
				Iterator<BlockPos> iterator = transmitters.iterator();
				MutableText message = new TranslatableText(TranslationKeys.MESSAGE_TRANSMITTERS_ACTIVE, frequencyComponent, transmitters.size());
				message.append("\n");

				while (true)
				{
					BlockPos transmitter = iterator.next();
					MutableText component = new LiteralText(String.format("[x: %d, y: %d, z: %d]", transmitter.getX(), transmitter.getY(), transmitter.getZ())).formatted(Formatting.YELLOW);
					...
					if (player.hasPermissionLevel(2))
					{
						...
						HoverEvent hover = new HoverEvent(HoverEvent.Action.SHOW_TEXT, new TranslatableText(TranslationKeys.MESSAGE_TELEPORT));
