...
public class TileFrequencyOld extends TileEntity implements Device.Block, IChunkLoadListener
{
	...
	public TileFrequencyOld(Type type)
	{
	...
	public CompoundNBT save(CompoundNBT nbt)
	{
		...
		if (type == Type.TRANSMITTER)
			nbt.putString("id", ModTiles.redstoneTransmitter.getRegistryName().toString());
		else if (type == Type.RECEIVER)
			nbt.putString("id", ModTiles.redstoneReceiver.getRegistryName().toString());
	...
	public void onChunkLoad()
	{
		...
		level.getServer().tell(new TickDelayedTask(1, this::setChanged));
	}
